            ...
            <a-col :xs="24" :lg="6">
                <a-form-item field="尺寸模式" label="模式详情 ">
                    <a-select size="large" v-model="uploadData.modeDetail">
                        <a-option v-for="item in modeDetail.data" :key="item.value" :value="item.value">{{
                            item.text
                            }}
                        </a-option>
                    </a-select>
        ...
        <a-row>
            <a-col>
                <a-button :disabled="responseData.data.imageName.length"
                          long status="success" type="primary" @click="submitImage()">上传服务器转码
                </a-button>
            </a-col>
        </a-row>
...
let modeDetail = reactive({
    data: [{value: "1.0f", text: "原图尺寸"}, {value: "0.75f", text: "原图75%"},
...
/** 上传后的回调 */
const successProcessImage = (resp) => {
